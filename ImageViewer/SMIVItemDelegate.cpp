...
void SMIVItemDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    bool itemIsSelected = option.showDecorationSelected && (option.state & QStyle::State_Selected);
    ...
    painter->drawRect(x0 + FrameWidth + Margin, y0 + FrameWidth + Margin, ThumbWidth + 1,  ThumbHeight + 1);

    // draw the image:
    const QImage& thumb = data->thumb();
    QPoint orig = QPoint(x0 + FrameWidth + Margin + 1, y0 + FrameWidth + Margin + 1);
    painter->drawImage(orig,  thumb, QRect ( 0, 0, thumb.width(),  thumb.height()));
